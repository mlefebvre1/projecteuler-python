...
def is_lynchrel_number(n: int) -> bool:
    ...
    for i in range(0, max_iter):
        original_plus_reverse = n + int(str(n)[::-1])
        if is_palindrome(original_plus_reverse):
            return False
        n = original_plus_reverse
    else:
        return True
...
def problem55():
    ...
    """
    max_n = 10000
    lynchrel_numbers = list(filter(lambda n: is_lynchrel_number(n), range(0, max_n)))
    return len(lynchrel_numbers)
